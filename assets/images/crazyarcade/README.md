...
| `character.png` | 플레이어 캐릭터 | 아래 설명 참고 |
| `bot.png` | 봇 캐릭터 | 캐릭터와 같은 배치 |
| `bot1.png` | 봇 캐릭터 (다른 생김새) | 선택 |
| `bot2.png` | 봇 캐릭터 (다른 생김새) | 선택 |
...
- 누가 플레이어인지는 발밑의 팀 색 고리로 구분됩니다.

### 봇 생김새 여러 개 쓰기

`bot.png` · `bot1.png` · `bot2.png` 중 넣어 둔 것들 안에서 봇마다 무작위로 골라 씁니다.
하나만 넣으면 모든 봇이 그것을 쓰고, 하나도 없으면 플레이어 그림을 함께 씁니다.
같은 시드에서는 같은 조합이 나오므로 재현이 가능합니다.
